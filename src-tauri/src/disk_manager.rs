...
impl DiskManager {
    pub fn global() -> &'static mut DiskManager {
        unsafe {
            DISKMANAGER.get_mut().expect("DiskManager is not initialized")
        }
    }
    ...
    pub fn new_directory_to_disk(&mut self, name: &str) -> Result<(), &'static str> {
        ...
        if let Some(_fcb) = self.cur_dir.get_fcb_by_name(name) {
            return Err("[ERROR]\tThere's already a file with a same name!");
        }
    ...
    // 通过FCB块找到目录项
    pub fn get_directory_by_fcb(&self, dir_fcb: &Fcb) -> Directory {
        pinfo();
    ...
    // 通过FCB块找到文件
    pub fn get_file_by_fcb(&self, fcb: &Fcb) -> Vec<u8> {
        pinfo();
    ...
    fn delete_file_by_fcb_with_index(
        ...
        if let FileType::Directory = fcb.file_type {
            ...
            if dir.files.len() > 2 {
                self.set_current_directory(&dir.name);
                for file in dir.files {
                    if file.name != "." && file.name != ".." {
                        self.delete_file_by_name(&file.name);
                    }
                }
    ...
    /// 在当前文件夹创建新文件并写入
    pub fn create_file_with_data(&mut self, name: &str, data: &[u8]) -> Result<(), &'static str> {
        pinfo();
    ...
    pub fn delete_file_by_name(&mut self, name: &str) -> Result<(), String> {
        let (index, fcb) = self.cur_dir.get_fcb_by_name(name).unwrap();
        pdebug();
        println!("Trying to delete file in dir file list...");
        
        let res = self.delete_file_by_fcb_with_index(&fcb.clone(), None);
    ...
    pub fn set_current_directory(&mut self, name: &str) {
        ...
        self.cur_dir = dir;
        if name == ".." {
            let pos = self.work_dir.rfind('/');
            if pos.unwrap() != 0 {
                self.work_dir = self.work_dir.clone()[0..pos.unwrap()].to_string();
            }
        } else if name != "." {
            self.work_dir.push_str((String::from("/") + name).as_str()) 
        }
    ...
    // 文件改名
    pub fn rename_file_by_name(&mut self, old: &str, new: &str) -> Result<(), &'static str> {
        if let Some(_fcb) = self.cur_dir.get_fcb_by_name(new) {
            return Err("[ERROR]\tThere's already a file with a same name!")
        }
    ...
    pub fn delete_space_by_name(&mut self, name: &str) -> Result<(), String> {
        let (index, fcb) = self.cur_dir.get_fcb_by_name(name).unwrap();
        if let Err(err) = self.delete_space_on_fat(fcb.first_cluster) {
            return Err(err)
        }
    ...
    // 文件编辑，将涉及到的fatItem设置为未使用，重新分配空间
    pub fn edit_file_by_name(&mut self, index: usize, old_fcb: &Fcb, new_name: &str, data: &[u8]) -> Result<(), &'static str> {
        if old_fcb.name == new_name {
            return Err("[ERROR]\tThere's already a file with a same name!")
        }
        let first_cluster = self.write_data_to_disk(data);
        let new_fcb = Fcb {
    ...
    pub fn paste_with_copy_by_fcb(&mut self, fcb: &Fcb, level: usize) -> Result<(), &'static str> {
        let mut name = fcb.name.clone();
        if level == 0 {
            let time = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs();
            name.push_str(format!("{}{}", String::from("的副本"), time.to_string()).as_str());
        }
        if let FileType::File = fcb.file_type {
            let data = self.get_file_by_fcb(&fcb);
            self.create_file_with_data(&name, data.as_slice())
        } else {
            let dir = self.get_directory_by_fcb(&fcb);
            self.new_directory_to_disk(&name);
            self.set_current_directory(&name);
            for file in dir.files {
                if file.name != "." && file.name != ".." {
                    self.paste_with_copy_by_fcb(&file, level+1);
                }
            }
            self.set_current_directory("..");
            Ok(())
    ...
    // 剪切（未完成）
    pub fn paste_with_cut_by_fcb(&mut self, fcb: &Fcb) -> Result<(), &'static str> {
        if let FileType::File = fcb.file_type {
            let dir = self.get_directory_by_fcb(&fcb);
        } else {
...
pub struct Fcb {
    ...
    pub first_cluster: usize, // 起始块号
    pub length: usize,        // 文件大小
}
...
impl Directory {
    ...
    pub fn get_fcb_by_name(&self, name: &str) -> Option<(usize, &Fcb)> {
        let mut res = None;
    ...
    // 获取当前文件夹文件（FCB）列表
    pub fn get_file_list(&self) -> Vec<Fcb> {
        self.files.clone()
    }
}
